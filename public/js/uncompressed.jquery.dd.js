   ...
   var dd = function(element, options)
   {
	...
	var createA = function(currentOptOption, current, currentopt, tp) {
		...
		if(options.useSprite!=false) {
		 ...
		 arrow = (arrow.length==0) ? "" : '<div class="img_thumb"><img src="'+arrow+'" align="absmiddle" /></div>';																 
		};
		//console.debug("clsName "+clsName);
		var sText = $(currentOptOption).text();
		var sTextHead = $(currentOptOption).attr('role');
		...
		aTag +=  ' id="'+aid+'">';
		aTag += arrow + '<div class="select-right"><h3>'+sTextHead+'</h3><p>' +sText+ '</p></div></a>';
		return aTag;
	};
	...
	var createTitleDiv = function () {
		...
		if(document.getElementById(elementid).options.length>0) {
			...
			sHeading = $("#"+elementid+" option:selected").attr("role");
		};
		//console.debug("sObj	 "+sObj.length);
		arrow = (arrow.length==0 || arrow==undefined || options.showIcon==false || options.useSprite!=false) ? "" : '<div class="img_thumb"><img src="'+arrow+'" align="absmiddle" /></div>';
		var sDiv = '<div id="'+titleid+'" class="'+styles.ddTitle+'"';
		sDiv += '>';
		sDiv += '<span id="'+arrowid+'" class="'+styles.arrow+'"></span><div class="'+styles.ddTitleText+'" dir="1" id="'+titletextid+'">'+arrow + '<div class="select-right"><h3>'+sHeading+'</h3><p>'+sText+'</p></div></div></div>';
		return sDiv;
	};
