	...
	$('.addtocart').on('click', function () {
        ...
        if (imgtodrag) {
			...
			.animate({
				...
				'left':win_width-(cart.width()/1.05),
				'width':cart.width()
            }, 1000 );
            imgclone.animate({
                'opacity':0
		...
		if($(this).hasClass('active')){
			$(this).removeClass('active');
			$(this).next('.order-history-sub').slideUp();
		}
		else{
			$('.order-tab').removeClass('active');
			$('.ord-hstr-div').find('.order-history-sub').slideUp();
			$(this).addClass('active');
			$(this).next('.order-history-sub').slideDown();
		}
	});
...
$('.plus').click(function () {
        total_qtly = $(this).parents('.qtlpluminus').text();
        total_qtly = parseInt(total_qtly) + 1;
        $('#cartquantity').val(total_qtly);
        $(this).parents('.qtlpluminus').find('.total-count').text(total_qtly);
    });
    $('.minus').click(function () {
        total_qtly = $(this).parents('.qtlpluminus').text();
        if (total_qtly > 1) {
            total_qtly = total_qtly - 1;
            $('#cartquantity').val(total_qtly);
            $(this).parents('.qtlpluminus').find('.total-count').text(total_qtly);
        }
    });
	...
	$('.account-link').on('click',function(){
		...
		if(actEvt.hasClass('active')){
			...
			actEvt.removeClass('active');
		}
		else{
			$('.account-main').find('.account-sub').addClass('opened');
			$('.account-main').find('.account-sub ul').addClass('show');
			actEvt.addClass('active');
		}
	});